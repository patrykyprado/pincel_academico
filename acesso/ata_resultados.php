...
include_once('includes/conectar_pdo.php');
...
//PEGA NOME DA BASE
$sql_base = mysql_query("SELECT id_base, base_ensino FROM ced_base_ensino WHERE id_base IN ($base_in)");
?>
...
<table width=\"100%\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" class=\"full_table_list\" style=\"font-size:7px; font-family:Arial, Helvetica, sans-serif; line-height:10px\">
    ...
    <tr class=\"diario_frequencia_header\">
    <td colspan=\"2\"><b>Curso:<br />".strtoupper($nivel_turma).": ".strtoupper($curso_turma)." - Ano/M�dulo ".strtoupper($modulo_turma)."</b></td>
...
//monta o header com titulos das colunas
  ...
  <td class=\"diario_frequencia_header\" align=\"center\" bgcolor=\"#F5F5F5\"><b>Nota</b></td>";
    ...
    while ($dados = $sql_alunos->fetch(PDO::FETCH_ASSOC)) {
        // enquanto houverem resultados...
        ...
        if(mysql_num_rows($sql_cancelados)>=1){
            ...
            $cancelado_observacao .= "Aluno(a) $nome, n� de matricula $codigo foi $nome_ocorrencia em $data_ocorrencia.<br>";
        }
        $i +=1;
...
WHERE matricula = $codigo AND turma_disc = $cod_tdisc2 AND subgrupo <> 'C' AND id_etapa = $etapa
			");
                ...
                if($nota_final1 <= $nota1){//verifica se a nota da recupera��o � menor que soma das notas
                    $nota_final1 = $nota1;
                }
                ...
                $falta         = 0;//$dados_falta["falta_total"];
